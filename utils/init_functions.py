...
def add_sim_conds(sim_conds):
    ...
    for x in ["UP", "DOWN"]:
        lid_cond[x]["Sflange_up"] = (
            ((lid_cond[x]["Dflange"] / 2) ** 2 - (lid_cond[x]["PDflange_up"] / 2) ** 2) * math.pi / 1000000
        )
        ...
        lid_cond[x]["Mflange"] = (lid_cond[x]["Vflange_up"] + lid_cond[x]["Vflange_dw"]) * drum_wall["Dense_drumw"]
    ...
    input_gass["dense_mean"] = (
        input_gass["dense_co2"] * input_gass["mf_co2"] + input_gass["dense_n2"] * input_gass["mf_n2"]
    )
    input_gass["c_mean"] = input_gass["c_co2"] * input_gass["mf_co2"] + input_gass["c_n2"] * input_gass["mf_n2"]
    input_gass["vi_mean"] = input_gass["vi_co2"] * input_gass["mf_co2"] + input_gass["vi_n2"] * input_gass["mf_n2"]
    input_gass["cp_mean"] = input_gass["cp_co2"] * input_gass["mf_co2"] + input_gass["cp_n2"] * input_gass["mf_n2"]
...
def init_stream_conds(sim_conds, stream, stream_conds):
    ...
    if stream == 1:
        tgt_stream_conds["r_out"] = (
            sim_conds["PACKED_BED_COND"]["Rbed"] / sim_conds["NUM_STR"] * stream + tgt_stream_conds["r_in"]
        )
    else:
        tgt_stream_conds["r_out"] = (
            sim_conds["PACKED_BED_COND"]["Rbed"] / sim_conds["NUM_STR"] * stream + stream_conds[stream - 1]["r_in"]
        )
    # ストリーム断面積
