...
class GasAdosorption_for_Optimize:
    ...
    def run_optimization(self):
        ...
        storage = optuna.storages.RDBStorage(
            ...
            engine_kwargs={"connect_args": {"timeout": 30}, "isolation_level": "SERIALIZABLE"},
        )
        study = optuna.create_study(
            study_name="GasAdsorption",
            directions=["minimize"] * self.num_objective,
    ...
    def _plot_outputs(self, study, df_opt):
        ...
        if self.num_objective != 1:
            ...
            sns.pairplot(df_tgt, hue="best_trial", diag_kws={"alpha": 0.5}, plot_kws={"alpha": 0.3})  # 散布図行列
        ...
        if num_state != 1:  # 状態変数が複数のときのみ
            ...
            plt.figure(figsize=(16, 16), tight_layout=True)
            pp = sns.pairplot(df_tgt, hue=f"best_trial", diag_kws={"alpha": 0.5}, plot_kws={"alpha": 0.3})  # 散布図行列
            ...
            for i, obj_name in enumerate(self.objective_name):
                ...
                pp = sns.pairplot(
                    df_tgt, hue=obj_name, diag_kws={"alpha": 0.5}, plot_kws={"alpha": 0.8}, palette="RdBu"
                )  # 散布図行列
                for ax in pp.axes.flat:
                    ax.set(xscale="log")
                    ax.set(yscale="log")
