...
def initial_adsorption(sim_conds, stream_conds, variables, series=False):
    ...
    for stream in range(1, 1 + sim_conds["NUM_STR"]):
        ...
        hb_dict[stream] = {}
        # sec_1は手動で実施
        mb_dict[stream][1] = base_models.material_balance_adsorp(
            sim_conds=sim_conds,
        ...
        hb_dict[stream][1] = base_models.heat_balance(
            sim_conds=sim_conds,
            stream_conds=stream_conds,
            stream=stream,
            section=1,
        ...
        # sec_2以降は自動で実施
        for section in range(2, 1 + sim_conds["NUM_SEC"]):
            mb_dict[stream][section] = base_models.material_balance_adsorp(
                sim_conds=sim_conds,
                stream_conds=stream_conds,
                stream=stream,
                section=section,
            ...
            hb_dict[stream][section] = base_models.heat_balance(
                sim_conds=sim_conds,
                stream_conds=stream_conds,
                stream=stream,
                section=section,
    ...
    # 2. 壁面熱バラ（stream = 1+sim_conds["NUM_STR"]）
    hb_wall[1] = base_models.heat_balance_wall(
        sim_conds=sim_conds,
        stream_conds=stream_conds,
        section=1,
        variables=variables,
        heat_output=hb_dict[sim_conds["NUM_STR"]][1],
        heat_wall_output=None,
    )
    for section in range(2, 1 + sim_conds["NUM_SEC"]):
        hb_wall[section] = base_models.heat_balance_wall(
            sim_conds=sim_conds,
            stream_conds=stream_conds,
            section=section,
            variables=variables,
            heat_output=hb_dict[sim_conds["NUM_STR"]][section],
            heat_wall_output=hb_wall[section - 1],
        )
    # 3. 上下蓋熱バラ
    for position in ["up", "down"]:
        hb_lid[position] = base_models.heat_balance_lid(
            sim_conds=sim_conds, position=position, variables=variables, heat_output=hb_dict, heat_wall_output=hb_wall
        )
    # 4. バッチ吸着後圧力変化
    total_press_after_batch_adsorp = base_models.total_press_after_batch_adsorp(
...
def stop_mode(sim_conds, stream_conds, variables):
    ...
    for stream in range(1, 1 + sim_conds["NUM_STR"]):
        ...
        hb_dict[stream] = {}
        # sec_1は手動で実施
        mb_dict[stream][1] = base_models.material_balance_valve_stop(stream=stream, section=1, variables=variables)
        hb_dict[stream][1] = base_models.heat_balance(
            sim_conds=sim_conds,
            stream_conds=stream_conds,
            stream=stream,
        ...
        # sec_2以降は自動で実施
        for section in range(2, 1 + sim_conds["NUM_SEC"]):
            mb_dict[stream][section] = base_models.material_balance_valve_stop(
...
def equalization_pressure_depressurization(sim_conds, stream_conds, variables, downflow_total_press):
    ...
    # 0. 上流管からの流入計算
    depress_output = base_models.total_press_after_depressure(
        sim_conds=sim_conds,
        variables=variables,
    ...
    # 4. 下流塔の圧力と流入量計算
    downflow_fr_and_total_press = base_models.downflow_fr_after_depressure(
        sim_conds=sim_conds,
        stream_conds=stream_conds,
        variables=variables,
        mb_dict=mb_dict,
...
def desorption_by_vacuuming(sim_conds, stream_conds, variables):
    ...
    # 0. 排気後圧力の計算
    vacuum_output = base_models.total_press_after_vacuum(sim_conds=sim_conds, variables=variables)
    # 1. マテバラ・熱バラ計算
    for stream in range(1, 1 + sim_conds["NUM_STR"]):
        mb_dict[stream] = {}
        hb_dict[stream] = {}
        mf_dict[stream] = {}
        # sec_1は手動で実施
        mb_dict[stream][1], mf_dict[stream][1] = base_models.material_balance_desorp(
            sim_conds=sim_conds,
            stream_conds=stream_conds,
            stream=stream,
            section=1,
        ...
        # sec_2以降は自動で実施
        for section in range(2, 1 + sim_conds["NUM_SEC"]):
            mb_dict[stream][section], mf_dict[stream][section] = base_models.material_balance_desorp(
                sim_conds=sim_conds,
                stream_conds=stream_conds,
                stream=stream,
                section=section,
    ...
    # 4. 脱着後の全圧
    total_press_after_desorp = base_models.total_press_after_desorp(
        sim_conds=sim_conds, variables=variables, mf_dict=mf_dict
    )
    # 出力
    output = {
        "material": mb_dict,  # マテバラ
...
def batch_adsorption_downstream(sim_conds, stream_conds, variables, series, inflow_gas, stagnant_mf):
    ...
    for stream in range(1, 1 + sim_conds["NUM_STR"]):
        ...
        mb_dict[stream][1] = base_models.material_balance_adsorp(
            ...
            section=1,
            variables=variables,
            inflow_gas=inflow_gas[stream][sim_conds["NUM_SEC"]],
