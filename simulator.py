...
class GasAdosorption_Breakthrough_simulator:
    ...
    def __init__(self, cond_id):
        ...
        df_sim_conds = pd.read_excel(
            const.CONDITIONS_DIR + self.cond_id + "/sim_conds.xlsx", sheet_name=["共通", "塔1", "塔2", "塔3"]
        )
    ...
    def calc_adsorption_process(
        self, mode_list, termination_cond_str, variables_tower, record_dict, timestamp, manual=False, filtered_x=None
    ):
        """プロセスpの各塔のガス吸着計算を行う
        ...
        if manual:
            ...
            while timestamp + timestamp_p <= termination_cond_str:
                ...
                if filtered_x is not None:
                    self._overwrite_state_vars(filtered_x=filtered_x, timestamp=timestamp + timestamp_p)
                # 各塔の吸着計算実施
    ...
    def calc_adsorption_mode_list(self, sim_conds, mode_list, variables_tower):
        ...
        if cond1 | cond2:
            ...
            for tgt_tower_num in range(1, 1 + self.num_tower):
                ...
                new_variables_tower[tgt_tower_num], record_outputs_tower[tgt_tower_num], _ = self.branch_operation_mode(
    ...
    def branch_operation_mode(self, sim_conds, stream_conds, mode, variables, other_tower_params=None):
        ...
        # 流通吸着_下流
            ...
            calc_output = models.flow_adsorption_downstream(
                sim_conds=sim_conds, stream_conds=stream_conds, variables=variables, inflow_gas=other_tower_params
            )
        # バッチ吸着_上流
        elif mode == "バッチ吸着_上流":
            calc_output = models.batch_adsorption_upstream(
                sim_conds=sim_conds, stream_conds=stream_conds, variables=variables, series=True
            )
        # バッチ吸着_下流
        elif mode == "バッチ吸着_下流":
        ...
        # 均圧_加圧
            ...
            calc_output = models.equalization_pressure_pressurization(
                sim_conds=sim_conds, stream_conds=stream_conds, variables=variables, upstream_params=other_tower_params
            )
