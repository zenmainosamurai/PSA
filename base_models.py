...
def material_balance_adsorp(
    sim_conds, stream_conds, stream, section, variables, inflow_gas=None, flow_amt_depress=None, stagnant_mode=None
):
    """任意セルのマテリアルバランスを計算する
        吸着モード
    ...
    if (section == 1) & (inflow_gas is None) & (flow_amt_depress is None):  # 最上流セルの流入ガスによる吸着など
        inflow_fr_co2 = (
            sim_conds["INFLOW_GAS_COND"]["fr_co2"] * sim_conds["dt"] * stream_conds[stream]["streamratio"] * 1000
        )
        inflow_fr_n2 = (
            sim_conds["INFLOW_GAS_COND"]["fr_n2"] * sim_conds["dt"] * stream_conds[stream]["streamratio"] * 1000
        )
    elif (section == 1) & (flow_amt_depress is not None):  # 減圧時の最上流セルのみ対象
        inflow_fr_co2 = (
            sim_conds["INFLOW_GAS_COND"]["mf_co2"]
            * flow_amt_depress
            * sim_conds["dt"]
        ...
        inflow_fr_n2 = (
            sim_conds["INFLOW_GAS_COND"]["mf_n2"]
            * flow_amt_depress
            * sim_conds["dt"]
            * stream_conds[stream]["streamratio"]
            * 1000
...
def heat_balance(
    ...
    if mode == 0:
        hw1, u1 = _heat_transfer_coef(
            sim_conds, stream_conds, stream, section, temp_now, mode, variables, material_output
        )
        ...
        # u1 = variables["heat_t_coef_wall"][stream][section]
        hw1, u1 = _heat_transfer_coef(
            sim_conds, stream_conds, stream, section, temp_now, mode, variables, material_output, vacuum_output
        )
    ...
    if section == sim_conds["NUM_SEC"]:
        Hbb = (  # 下蓋への熱流束
            hw1 * stream_conds[stream]["Sstream"] * (temp_now - variables["temp_lid"]["down"]) * sim_conds["dt"] * 60
        )
    else:
        Hbb = u1 * Abb * (temp_now - temp_below_cell) * sim_conds["dt"] * 60  # 下流セルへの熱流束
    # 上流セルヘの熱流束 [J]
    if section == 1:  # 上蓋からの熱流束
        Hroof = hw1 * stream_conds[stream]["Sstream"] * (temp_now - variables["temp_lid"]["up"]) * sim_conds["dt"] * 60
    ...
    # 熱電対上昇温度 [℃]
    temp_increase = heat_flux * sim_conds["dt"] * 60 / heat_capacity
    # 次時刻熱電対温度 [℃]
...
def heat_balance_lid(sim_conds, position, variables, heat_output, heat_wall_output):
    ...
    if position == "up":
        ...
        Hlidall_heat = (
            heat_output[2][sim_conds["NUM_SEC"]]["Hroof"]
            - heat_output[1][sim_conds["NUM_SEC"]]["Hroof"]
            - Hout
            - heat_wall_output[sim_conds["NUM_SEC"]]["Hbb"]
        )
    # セクション到達温度 [℃]
    args = {
        "sim_conds": sim_conds,
...
def total_press_after_vacuum(sim_conds, variables):
    ...
    for iter in range(_max_iteration):
        ...
        # 真空ポンプ排気レイノルズ数
        Re = rho * linear_velocity * sim_conds["VACUUMING_PIPE_COND"]["Dpipe"] / mu
        # 真空ポンプ排気管摩擦係数
...
def total_press_after_depressure(sim_conds, variables, downflow_total_press):
    ...
    for iter in range(_max_iteration):
        ...
        flow_rate = (
            dP
            * sim_conds["PRESS_EQUAL_PIPE_COND"]["Dpipe"] ** 2
            / (32 * mu * sim_conds["PRESS_EQUAL_PIPE_COND"]["Lpipe"])
        )
        flow_rate = max(1e-8, flow_rate)
        # 均圧管レイノルズ数
        Re = rho * abs(flow_rate) * sim_conds["PRESS_EQUAL_PIPE_COND"]["Dpipe"] / mu
        # 管摩擦係数
        lambda_f = 64 / Re if Re != 0 else 0
        # 均圧配管圧力損失 [MPaA]
        P_resist = (
            lambda_f
            * sim_conds["PRESS_EQUAL_PIPE_COND"]["Lpipe"]
            / sim_conds["PRESS_EQUAL_PIPE_COND"]["Dpipe"]
            * flow_rate**2
            / (2 * 9.81)
        ) * 1e-6
        # 収束判定
    ...
    ### 次時刻の圧力計算 ----------------------------------------
    # 容器上流空間を移動する物質量 [mol]
    mw_upper_space = flow_amount_m3_N * 1000 * sim_conds["dt"] / 22.4
    # 上流側の合計体積 [m3]
...
def total_press_after_batch_adsorp(sim_conds, variables, series):
    ...
    # 圧力変化量
    diff_pressure = R * temp_mean / V * (F / 22.4) * sim_conds["dt"] / 1e6
    # 変化後全圧
...
def _heat_transfer_coef(
    sim_conds, stream_conds, stream, section, temp_now, mode, variables, material_output, vacuum_output=None
):
    """層伝熱係数、壁-層伝熱係数を算出する
    ...
    Phi_2 = 0.07
    # Yagi-Kunii式 3
    Phi = Phi_2 + (Phi_1 - Phi_2) * (sim_conds["PACKED_BED_COND"]["epsilon"] - 0.26) / 0.26
    # Yagi-Kunii式 4
    hrv = (
        0.227
        / (
    ...
    # Yagi-Kunii式 7
    ke0_kf = sim_conds["PACKED_BED_COND"]["epsilon"] * (1 + hrv * sim_conds["PACKED_BED_COND"]["dp"] / kf) + (
        1 - sim_conds["PACKED_BED_COND"]["epsilon"]
    ) / (1 / ksi + 2 * kf / 3 / kp)
    # 静止充填層有効熱伝導率 [W/m/K]
    ke0 = kf * ke0_kf
    ...
    # 流入ガス体積流量 [m3/s]
    if mode == 0:
        f0 = (material_output["inflow_fr_co2"] + material_output["inflow_fr_n2"]) / 1e6 / (sim_conds["dt"] * 60)
...
def __optimize_temp_reached(
    temp_reached, sim_conds, stream_conds, gas_cp, Mgas, temp_now, Habs, Hwin, Hwout, Hbb, Hroof, stream
):
    ...
    # 充填層が受け取る熱(ΔT基準) [J]
    Hbed_time = (
        sim_conds["PACKED_BED_COND"]["Cbed"]
        * stream_conds[stream]["streamratio"]
        / sim_conds["NUM_SEC"]
...
def __optimize_temp_reached_lid(temp_reached, sim_conds, temp_now, Hlidall_heat, position):
    ...
    """
    # 壁が受け取る熱(ΔT基準) [J]
    Hlid_time = sim_conds["DRUM_WALL_COND"]["sh_drumw"] * (temp_reached - temp_now)
    if position == "up":
